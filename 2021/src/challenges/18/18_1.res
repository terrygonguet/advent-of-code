...
let default = (inpt: string) => {
  ...
  let pairs = inpt->String2.split("\n")->Array2.map(tokenize)
  // ->Array2.map(parse)
  // ->Utils.unwrapOptionArray
  // ->Array2.map(magnitude)

  let first = pairs->Array2.shift->Option.getUnsafe
  let result = pairs->Array2.reduce((acc, cur) => add(acc, cur), first)
  (result->stringify, result->parse->Option.map(magnitude))
}
