(* Notes for future me *)
(* The idea to solve part 2 is to find all starting points and follow
   the paths until we come back to start in a multiple of path length.
   We keep track of how long it takes to get to ending points and
   those are the cycles we have to align with math (like gcd or whatever) *)
